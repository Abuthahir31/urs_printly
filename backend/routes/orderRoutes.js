...
async function uploadImageToFirebase(base64String) {
  ...
  const filename = `custom-orders/${Date.now()}-${Math.round(Math.random() * 1E9)}.${extension}`;
  ...
  try {
    ...
    if (Array.isArray(product)) {
      for (const p of product) {
        ...
        if (p.customImage && p.customImage.startsWith('data:')) {
          try {
            customImageUrl = await uploadImageToFirebase(p.customImage);
          } catch (uploadErr) {
        ...
        items.push({
          ...
          name: p.name,
          price,
          basePrice: p.basePrice || price,
          qty,
          image: p.image,
          customNames: Array.isArray(p.customNames) ? p.customNames : [],
          customImage: customImageUrl || null,
          selectedProductType: p.selectedProductType || null,
        });
